...
You will learn how to deploy two microservices in Open Liberty containers to an OpenShift 4 cluster running locally on your computer using CodeReady containers.
To learn how to deploy to an OpenShift 4 cluster using operators, see the https://openliberty.io/guides/cloud-openshift-operator.html[Deploying microservices to OpenShift by using Kubernetes Operators^] guide. 
...
Red Hat CodeReady Containers is a tool that allows you to quickly build and run
a minimal OpenShift 4 cluster on your local computer. This simplifies setup 
and testing while providing all of tools needed to develop container-based
applications. To read more about CodeReady Containers, check out the
https://access.redhat.com/documentation/en-us/red_hat_codeready_containers/1.0/html/getting_started_guide/getting-started-with-codeready-containers_gsg[official
documentation].
...
- *CodeReady Containers:* You need to install CodeReady Containers (CRC) to run
OpenShift 4 locally on your computer. For installation instructions refer to the
official https://access.redhat.com/documentation/en-us/red_hat_codeready_containers/1.0/html/getting_started_guide/getting-started-with-codeready-containers_gsg[CodeReady Containers documentation].   
...
If the cluster started successfully, you should see something similar to the following output:
...
To login as a cluster admin, run 'oc login -u kubeadmin -p APEEh-jjrVy-hL4Z1-CI9Kj https://api.crc.testing:6443'.
...
CRC already includes the `oc` binary. To use the `oc` commands, run the
...
Next, create a new OpenShift project by running the following command. Replace `[project-name]` with a name for your project:
...
You are now ready to build and deploy microservices.
...
In order to use the OpenShift registry with Docker, you must add the registry domain to your Docker Engine's list of insecure registries. 

include::{common-includes}/os-tabs.adoc[]

[.tab_content.mac_section]
--
Click the Docker icon in your menu bar, select *Preferences*, and then click *Docker Engine*. If there is an existing JSON object, add the following key/value pair:
...
If there is no existing JSON object, add the following object: 
...
Next, press the *Apply & Restart* button for the the changes to take effect. 
--

[.tab_content.windows_section]
--
Click the Docker icon in your system tray, select *Settings*, and then click *Docker Engine*. If there is an existing JSON object, add the following key/value pair:
...
If the file doesn't already contain a JSON object, add the following objet:
...
To learn more about testing insecure registries, check out the official https://docs.docker.com/registry/insecure/[Docker documentation] on it. 
...
Hostnames are in the `inventory-route-[project-name].apps-crc.testing` format. Ensure that
...
`[project-name]` with the appropriate value:

[role=command]
```
mvn verify \ 
-Dsystem.ip=system-route-[project-name].apps-crc.testing \
-Dinventory.ip=inventory-route-[project-name].apps-crc.testing
```
