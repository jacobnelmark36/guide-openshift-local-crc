...
You'll learn how to deploy two microservices in Open Liberty containers to an OpenShift 4 cluster running locally on your computer using CodeReady Containers.
...
Using the cloud let's you easily scale and manage your microservices in a high-availability setup.
...
https://access.redhat.com/documentation/en-us/red_hat_codeready_containers/1.23/html/getting_started_guide/index[official
documentation].

The two microservices you'll deploy are called `system` and `inventory`. 
...
[role="no_copy"]
----
SET PATH=C:\Users\JAMIEColeman\.crc\bin\oc;%PATH%
...
You'll use OpenShift's integrated container image registry called OpenShift Container Registry (OCR). 
...
Add the OpenShift registry domain to your Docker Engine's list of insecure registries to use it.

include::{common-includes}/os-tabs.adoc[]

[.tab_content.mac_section]
--
Click the Docker icon in your menu bar, select *Preferences*, and then click *Docker Engine*. If there's an existing JSON object, add the following key/value pair:
...
To learn more about testing insecure registries, check out this related official https://docs.docker.com/registry/insecure/[Docker documentation]. 
--

[.tab_content.windows_section]
--
Click the Docker icon in your system tray, select *Settings*, and then click *Docker Engine*. If there's an existing JSON object, add the following key/value pair:
...
If you're using a custom credential store for securing your registry credentials, or if you're unsure where your credentials are stored, use the following command:
...
also be deleted from the cluster by using the same YAML file that you used to deploy them. If you're interested 
...
Hostnames are in the `inventory-route-my-project.apps-crc.testing` format. Ensure that
you're in your project, not the `default` project, which is shown in the upper-left corner of the web console.

Enter the following URLs into your web browser to access your microservices. Substitute the appropriate hostnames
