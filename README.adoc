...
running locally on your computer using CodeReady Containers.
To learn how to deploy to an OpenShift 4 cluster using operators, see the 
...
It also returns the pod’s name in the HTTP header, making replicas more distinguishable from each other.
...
Save this output as it may be required later in this guide. 
...
If prompted, use the developer password from when you ran `crc start` earlier.
...
Each microservice resides in its own directory, `start/system` and `start/inventory` respectively. 
Both of these directories contain a Dockerfile, which is necessary for building the Docker images. 
...
The web console URL is similar to `https://console-openshift-console.apps-crc.testing` and was given as part of
...
After logging in, you can view the routes by navigating to the `Networking > Routes` page on the left. 
Hostnames are in the `inventory-route-my-project.apps-crc.testing` format and are in the `Location` column.
Ensure that you're in your project, not the `default` project, 
which is shown in the upper-left corner of the web console.
